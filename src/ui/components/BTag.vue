...
const src = computed(() => {
    ...
    return bra
        ? `https://${VITE_SERVER}/tag/gtin/${tag.value.gtin}?ui=extension-popup&bra=${encodeURIComponent(bra)}&theme=${theme.name.value}`
        : `https://${VITE_SERVER}/tag/${tag.value._id}?ui=extension-popup&theme=${theme.name.value}`
})
onMounted(() => {
