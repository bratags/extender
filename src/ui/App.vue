<template>
    <v-app :theme="theme">
        <v-app-bar density="compact" flat>
            <v-btn variant="plain" icon density="compact" color="quaternary" v-if="route?.path !== 'main'"
                @click="routeHandler('main')">
                <v-icon class="mr-2" icon="close"></v-icon>
            </v-btn>
            <v-spacer></v-spacer>
            <span class="mx-8 text-body-2 font-weight-thin text-quaternary">v{{ version }}</span>
        ...
        <v-footer :color="theme === 'light' ? 'grey-lighten-4' : undefined" app class="pa-4 d-flex align-center">
            <a id="site-href" target="_blank" rel="noopener" style="text-decoration: none" href="https://bratags.com">
                <div class="text-h6 text-quaternary ml-2">
                    <span style="font-family: sans-serif; font-size: smaller">©</span>
                    2023 Bratags
                </div>
            </a>
            <v-spacer></v-spacer>
